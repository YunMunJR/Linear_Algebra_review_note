...
A set $F$ is called a \emph{field} if we can define two operators $+$ and $\cdot$ s.t.
...
\begin{description}
\item[1.] The sets $\mathbb{R}$, $\mathbb{C}$, $\mathbb{Q}$ are \emph{fields}.
\item[2.] $M_n(\mathbb{R})$ = \{n by n matrices with entries in $\mathbb{R}$\} is not a \emph{field}. (Mutiplication of matrices is not commutative in general $AB \neq BA$)
\item[3.] $\mathbb{F}_{2}$ = \{0, 1\} is a \emph{field}.
...
\[
\frac{a}{1}\coloneqq a, \qquad \forall a\in\mathbb{F}_2.
\]

\[
\mathbb{F}_2 \text{ satisfies (A0--A4), (M0--M4), and (P1).}
\]
\end{description}
...
\subsection{Definition}
Let $F$ be a $field$. A set $V$ is called a \emph{vector space} over $F$ if we can endow with two operations : addition and scalar multiplication by $F$ s.t.
\begin{description}
...
\begin{description}
\item[1.] $V = \mathbb{R}^n$ is a vector space over $\mathbb{R}$\\ \textcolor{red}{(add on $\mathbb{R}^n$, scalar multiplication by $\mathbb{R}$)}\\ In this case, zero vector $\mathbf{0}$ is given by $(0, 0,\ldots, 0)$\\ In general, let $F$ be a \emph{field} ($\mathbb{R}$ or $\mathbb{Q}$ or $\mathbb{C}$ or $\mathbb{F}_2 \ldots$), 
\[
F^n
\coloneqq
\{(x_1,\ldots,x_n):x_i\in F\}
\]
\item[2.] Let $F$ be a \emph{field}. Define $F[x]$
\[
F[x]
\coloneqq
\left\{
...
&= \mathbf{0} + \mathbf{v^{\prime\prime}} = (\mathbf{v^\prime} + \mathbf{v}) + \mathbf{v^{\prime\prime}}\\ 
&= \mathbf{v^\prime} + (\mathbf{v} + \mathbf{v^{\prime\prime}}) = \mathbf{v^\prime} + \mathbf{0}\\
&= \mathbf{v^\prime}\\
\end{aligned}
\]
...
\begin{aligned}
\mathbf{w} + \mathbf{w}
&= 0 \cdot \mathbf{v} + 0 \cdot \mathbf{v} = (0 + 0) \cdot \mathbf{v} = 0 \cdot \mathbf{v} = \mathbf{w}\\
\end{aligned}
\]
\[
\begin{aligned}
\mathbf{w} + \mathbf{w} + \mathbf{(-w)} = \mathbf{w} + \mathbf{(-w)} \implies \mathbf{w} = \mathbf{0}\\
\end{aligned}
\]
...
\subsection{Definition}
Let $V$ be a vector space over $F$. A subset $W$ of $V$ is called a \emph{subspace of V} if $W$ is also a vector space over $F$ with addition and scalar multiplication inherited from $V$.

\subsection{Theorems}
\textbf{Theorem 1.5.1.}

Let $V$ be a vector space over $F$ and let $W \subset V$. Then $W$ is a subspace of $V$ iff. the following are satisfied:
\begin{itemize}
    \item $W$ is not empty;
...
Let $A$ be an $m\times n$ matrix. Then the solution space
\[
...
\begin{enumerate}
    ...
    \]
    Therefore $\mathbf{u + v}\in W$. Hence $W$ is closed under addition.

    \bigskip

    \item Let $\mathbf{v}\in W$ and $c\in\mathbb{R}$.
    Since $A\mathbf{v} = \mathbf{0}$,
    \[
    A(c\mathbf{v}) = c(A\mathbf{v}) = c\mathbf{0} = \mathbf{0}.
    \]
    Therefore $c\mathbf{v}\in W$. Hence $W$ is closed under scalar multiplication.

    \bigskip

\end{enumerate}

Since the above three conditions are satisfied, $W$ is a subspace of $\mathbb{R}^n$.
\end{proof}
